...
def load_keywords(path: Path) -> List[str]:
    """Load keywords from a file path using existing reader utilities."""
...
def build_config_from_flags(SearchConfig, defaults, include_organic, include_paa, include_related, include_ads, include_ai_overview, selected_sites, allow_duplicates, max_pages, results_per_page):
    """Build a SearchConfig from CLI flags and defaults."""
    cfg_max_pages = max_pages if max_pages is not None else defaults.get('max_pages', 10)
    cfg_rpp = 10
    return SearchConfig(
        country_code=None,
        language_code=None,
...
def deduplicate_and_log(keywords: List[str], config, log_path: Path) -> Tuple[List[str], List[str]]:
    """Deduplicate normalized keywords and append structured entries to a log file."""
